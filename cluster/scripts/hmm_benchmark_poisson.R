...
library(pacman)
p_load(STAN, GenomicRanges, ggplot2, reshape, dplyr, gridExtra, cowplot)
...
nStates = 12
method = "PoissonLogNormal"
maxIters = 500


celltype_lst = list(cell_type=grep(cell_line, names(sample)))
...
line_1 = ggplot(data, aes(x=iterations, y=log_L)) +
   ...
   labs(x = "Iterations", y = "log-Likelihood", title=paste0(cell_line, "_", method, "::", "All iterations"))
      
line_2 = ggplot(data, aes(x=iterations, y=log_L)) +
   geom_line()+
   geom_point(alpha=0.2, color = "red")+
   labs(x = "Iterations", y = "log-Likelihood",  title=paste0(cell_line, "_", method, "::", "Iterations 1-10 omitted")) +
